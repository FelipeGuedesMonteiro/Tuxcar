		...
		<header role="banner" class="page-header">
			<?php if ( have_posts() ) : ?>
				<h1 class="search-title">
					<?php /* translators: %s: search term */ printf( esc_html__( 'Search Results for: %s','sayara-automotive'), '<span>' . esc_html( get_search_query() ) . '</span>' ); ?></h1>
			<?php else : ?>
				<h1 class="page-title"><?php esc_html_e( 'Nothing Found', 'sayara-automotive' ); ?></h1>
			<?php endif; ?>
		</header>
		<div class="row">
			<?php
		    ...
		    if($layout_setting == 'Left Sidebar'){ ?>
				...
				<div class="col-lg-8 col-md-8">
					...
					if ( have_posts() ) :
						...
						<p><?php esc_html_e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'sayara-automotive' ); ?></p>
						<?php
							get_search_form();
